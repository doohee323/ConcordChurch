...
public class ResourceService extends Service {
  ...
  public void launchWebView(WebView myWebView) {
    try {
      nwStatus = DeviceProperties.getConnectedState(mContext);
  ...
  public JSONArray getAllResources() {
    try {
      if (allResources.length() == 0) {
        ...
        String resources = ResourceService.STORAGE_DIR + "/resources.json";
        if (new File(resources).exists()) {
          String src =
              FileUtil.getFromFile(ResourceService.STORAGE_DIR + "/resources.json").toString();
